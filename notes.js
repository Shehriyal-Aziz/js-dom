...
element.innerHTML =
  "<b>new text</b> selected element k andar wale elements ko change krta h ";
...
img.setAttribute(
  "src",
  "https://images.unsplash.com/photo-1759420740201-7e7af435ca80?w=500&auto=format&fit=crop&q=60&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxmZWF0dXJlZC1waG90b3MtZmVlZHwyfHx8ZW58MHx8fHx8"
);
...
// things to study
// store/save  =>
localStorage.setItem("name", "sherry");
// data fetch kese karain =>
localStorage.getItem("name");
// remove
localStorage.removeItem("name");
// update
localStorage.setItem("name", "shehriiyal"); //set dono kam krta h set mean agr name bana hua h to update karo or agr nhi bana hua to set karo
// clear
localStorage.clear();
...
// clear
sessionStorage.clear(); //clear all if want one to clear use remove

// cookies
// browser mn small dat astore kne k liye use hota h arround 5kb but local and sesson has 5mb storage
// jo bhi data browser pr hoga wo reload krne pr server py jaai ga 
// things to study
// set (you can only set delete and read k liye koi keyword nhi)
document.cookie = "name","harsh";
// to delete, i wale icon ko click karo in url => manage cookie => delete
// to read, download extension "edit this cookie"


// note 
// local storage mn sirf string hi store hoti h, array and obj store nhi hote isliye onhe string banake store krte hn. using keyword "JSON.stringify" aap obj ya array ko string banate hn. string se usy apne natural form mn laane k liye JSON.parse ka use krte hn.
// steps
// pehele local storage mn obj ya array string banate hn aur stringify keyword mn wrap kr dete hn then usy var mn store kiya jata h q k var array and obj store kr sakta h isliye wahan parse keyword ka istamal krte hn to reverse

// array
 localStorage.setItem("frnd",JSON.stringify(["ali","aaban",'amir']));
let fn = JSON.parse(localStorage.getItem("frnd"));
console.log (fn);
// object
localStorage.setItem("fruits",(JSON.stringify({"1":"apple","2":"banna","3":"mango",})));
